...
import { ChefHat, Mail, Lock, AlertCircle, Eye, EyeOff, UserPlus, LogIn, ArrowRight } from 'lucide-react';
...
export const StaffLogin: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Staff profile registered successfully! You can now sign in.');
      setActiveTab('signin');
      ...
      <div className="hidden lg:flex w-[460px] bg-tomato text-white p-16 flex-col justify-between relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 my-auto space-y-6">
          <h1 className="text-5xl font-extrabold font-display leading-[1.1] tracking-tight">
            Kitchen dispatch logs.
          </h1>
          <p className="text-white/90 text-sm leading-relaxed max-w-[280px]">
            Access oven logs, update dispatch lists, and manage order statuses on the prep desk.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
        <div className="max-w-[440px] w-full">
          ...
          >
            {/* Header Title */}
            <div className="mb-8">
              <div className="flex items-center gap-2 text-amber-600 font-semibold text-xs font-mono tracking-wider mb-2 uppercase">
                <ChefHat size={14} />
             ...
             {isDemoMode ? (
              activeTab === 'signup' && (
                <div className="mb-5 p-3 bg-amber-50 border border-amber-100 text-amber-900 rounded-xl text-xs font-mono">
                  ✨ Quick Demo Mode: Staff register instantly to local secure storage.
                </div>
              )
            ) : (
              <div className="mb-5 p-4 bg-emerald-50 border border-emerald-100 text-emerald-950 rounded-2xl text-xs space-y-1.5 font-sans">
                <div className="font-bold text-emerald-800 flex items-center gap-1.5">
                  ...
                  Live Storefront Network Connected
                </div>
                <p className="text-slate-600 leading-relaxed text-[11px]">
                  Demo accounts are not pre-seeded on the live network. Please click the <strong className="text-tomato font-bold">Register Staff</strong> tab above to create your staff account.
                </p>
              </div>
            )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="staff@pizzaflow.com"
              ...
              <form onSubmit={handleSignUp} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      id="staff-signup-fullname"
                      type="text"
                      required
                      value={fullName}
                      onChange={(e) => setFullName(e.target.value)}
                      placeholder="Oven Specialist"
                      className="block w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-tomato focus:bg-white transition-all"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      placeholder="kitchen@pizzaflow.com"
                      className="block w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-tomato focus:bg-white transition-all"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    <button
                      id="btn-toggle-staff-signup-password"
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600"
                    >
